...
result <- cleanup_folders(folders, conf)
if(file.exists(conf)) unlink(conf, recursive = TRUE)
```

```{r build_notes, include=FALSE, eval=FALSE}
# 1. After rendering this Rmd to HTML, remove the html file, since all we want 
#    is the MD file. Then edit the MD file and remove the "r" after "```" with 
#    a search-and-replace. These Bash commands will accomplish these steps:
#      rm -f README.html
#      perl -pi -e 's/```r/```/g' README.md
